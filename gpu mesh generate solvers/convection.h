...
template<class T> class DC_Center :public I_Convection<T>{
public:
	virtual CSR<T> apply(vector<T> u_f, vector<T> v_f, CellField<T> phi, Mesh<T> mesh){
		CSR<T> eq(mesh);
		//copy(eq->A.begin(), eq->A.end(), ostream_iterator<T>(cout, " "));
		...
		for (int f = 0; f < mesh.iF.size(); f++) {
			...
			T mdot = u_f[inerF] * Sf_x + v_f[inerF] * Sf_y;
			//cout << mdot << endl;
			double a_on = VectorMath<T>::myMin(mdot, 0);
			double a_oo = VectorMath<T>::myMax(mdot, 0);
			eq.A[mesh.ON[f * 2 + 0]] = a_on;
			eq.A[mesh.IA[owner]] += a_oo;
			eq.b[owner] += a_oo*phi.inner[owner] + a_on*phi.inner[neighbor] - mdot;//Deffer correction


			double a_no = VectorMath<T>::myMin(-mdot, 0);
			double a_nn = VectorMath<T>::myMax(-mdot, 0);
			eq.A[mesh.ON[f * 2 + 1]] = a_no;
			eq.A[mesh.IA[neighbor]] += a_nn;

			eq.b[neighbor] += a_nn*phi.inner[neighbor] + a_no*phi.inner[owner] + mdot;//Deffer correction
			//copy(eq->A.begin(), eq->A.end(), ostream_iterator<T>(cout, " "));
		...
		for (int pat = 0; pat < mesh.bFbegin.size() - 1; pat++) {
			for (int f = mesh.bFbegin[pat]; f < mesh.bFbegin[pat + 1]; f++) {
				...
				T a_oo = VectorMath<T>::myMax(mdot, 0);
				T a_on = VectorMath<T>::myMin(mdot, 0);
				...
				eq.b[owner] += a_oo*phi.inner[owner] + a_on*phi.boundary[f] - mdot;
			}
		}
